...
For $n$ slots, picking $x$ *non-adjacent* items is the same as choosing $x$ positions out of $n - (x - 1)$ with *no restrictions*.


$S_{n,k}$ is a set of non-negative integer *k*-tuples $(x_1,x_2,\ldots x_k)$ with $x_1 + x_2 +\ldots x_k \leq n$ 
introduce the slack variable $x_{k+1} = n - (x_1+x_2+\ldots x_k)$ then $x_1 + x_2 +\ldots x_k +x_{k+1}= n$
so we have $k+1$ non-negative integers summing to $n$.

*forward map from $s_{n,k}$ to binary string*,
build a binary string with $n$ zeros and $k$ ones as follows :
$x_1$ zeros then a $1$, $x_2$ zeros then a $1,\ldots$ $,x_k$ zeros then a $1,$ finally $x_{k+1}$ zeros,
this produces a binary string of length $n+k$ with $n$ zeros and $k$ ones.

*inverse map from binary string to $s_{n,k}$*,
given any binary string with $n$ zeros and $k$ ones, read it as alternating block of zeros separated by $k$ ones. let the count of zeros before the first $1$, between the $1$'s, after the last $1$ be $(x_1,x_2,\ldots x_{k+1})$. then $(x_1,x_2,\ldots x_k)$ is a solution to $S_{n,k}$ because $x_1 + x_2 + \ldots x_k = n - x_{k+1} \leq n$.

$S_{n,k} = \binom{n+k}{k}$ 


$L_{n,k} = y_1\leq y_2\leq\ldots \leq y_k$ 

show bijection between $S_{n,k}$ and $L_{n,k}$ ,

take a climbing sequence $y$ and turn it into *step sizes* between values.
- if the sequence goes from $0$ to $2$ , then from $2$ to $5$, the steps are $2,3$.
- those step sizes always add up to at most $n$.

$L\to S$ , take differences
- $x_1 = y_1$,
- $x_2 = y_2 - y_1$ ,
- $\ldots$ 
- $x_k = y_k - y_{k-1}$ 

all these are non-negatives because $y$'s don't do down.
so, $x_1 + x_2 + \ldots x_k = y_k \leq n$ 

$S\to L$ , take cumulative sums
- $y_1 = x_1$ 
- $y_2 = x_1 + x_2$ 
- $\ldots$ 
- $y_k = x_1 +\ldots x_k$ 

this creates a weakly increasing sequence since you keep adding non-negative integers.


given a code of length $n-2$ , $(c_1,c_2,\ldots c_{n-2})$
each number is between $1$ and $n$ .

*1. count how many times each number appears* ,
- start with degree $=1$ 
- add $+1$ every time $v$ appears in the code
- this tells you how many edges each vertex must end up with.

*2. build edges one by one*, 
- repeat this for each entry in the code
- look at all vertices. find the smallest one that has degree $=1$ (the smallest leaf)
- connect that leaf to the next number in the code.
- reduce both their degrees.

at the end two vertices will be left with degree $=1$ . just connect them.
total trees $n^{n-2}$ .
...
*if* $\langle f_0,f_1,f_3,\ldots\rangle \leftrightarrow F(x)$ *then* $\langle c f_0,c f_1,c f_3,\ldots\rangle \leftrightarrow c F(x)$ .

*addition rule* ,

*if* $\langle f_0,f_1,f_3,\ldots\rangle\leftrightarrow F(x)$ *and*$\langle g_0,g_1,g_3,\ldots\rangle\leftrightarrow G(x)$ *then* $\langle f_0+g_0,f_1+g_1,f_2+g_2,\ldots\rangle\leftrightarrow F(x)+G(x)$ 

*right-shift rule* ,
*if* $\langle f_0,f_1,f_3,\ldots\rangle \leftrightarrow F(x)$ *then* $\langle  \overbrace{0,0,\ldots,0}^{k\ zeroes},f_0, f_1, f_3,\ldots\rangle \leftrightarrow x^k\cdot F(x)$ .

*differentiation* , 

differentiating a generating function has two effects on the corresponding sequence : each term is multiplied by its index and the entire sequence is shiften left one place.

*derivative rule* ,
*if* $\langle f_0,f_1,f_3,\ldots\rangle \leftrightarrow F(x)$ *then* $\langle  f_1, 2f_2,3f_3,\ldots\rangle \leftrightarrow F'(x)$ .
...
use right-shift rule to negate left shift effect of derivative rule.
...
the probability of a sample point is the product of the probabilities of edges on the path of the tree leading to the sample point.
...
so the leaves of the tree, gives us the probability that events $a_1,a_2,\ldots...a_n$(consider them as edge labels on a path to leaf) occured simultaneously.
...
if $A$ and $B$ are disjoint, then occurence of $B$ tells us that $A$ definitely didn't occured, so $A$ is not independent of $B$.
...
if $A$ is independent of $B$ , then  $P(A\cap B) = P(A)\cdot P(B)$ 

independence in more than two events,

if the probability of a event given that any subset of the rest of the events in the set occured is the same as probability of that event , then that event is independent of others. 
...
random variable is a function $R: S\to \mathbb{R}$ that maps each sample point to a real value.

*indicater/bernoulli/characteristic r.v* - *r.v with range $\{0,1\}$*

$\{\omega\ |\ R(\omega)=x\}$ is the event that $R=x$ 

$P_r(R=x) = \sum\limits^{\omega : R(\omega)=x} P_r(\omega)$ , 

probability that value of a random variable $R$ is $x$ is equal to sum of probabilities of all the sample points that are mapped to value $x$ by $R$.

$P_r(R\geq 2) = \sum\limits_{i=2}^{\infty} P_r(R=i)$
...
*if $G_1 = (V_1,E_1)$ and $G_2 = (V_2,E_2)$ are two graphs, then we say that $G_1$ is **isomorphic** to $G_2$ iff there exists a bijection $f: V_1\to V_2$ such that for every pair of vertices $u,v \in V_1:$*

$\{u,v\}\in E_1\ \ \text{iff}\  \ \{f(u),f(v)\}\in E_2$

the function $f$ is called an *isomorphism* between $G_1$ and $G_2$ .

two graphs  are isomorphic if they are the same up to a relabeling of their vertices.